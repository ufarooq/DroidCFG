...
public class SimpleNode {
    ...
    public String toString() {
        if (callTarget instanceof CallTarget.Method) {
            ...
            return tMethod.getElement().getContainingFile().getName() + ":" + tMethod.getElement().getContainingClass().getName() + ":" + tMethod.getElement().getName();
        ...
        if (callTarget instanceof CallTarget.Lambda) {
            ...
            //UMethod uMethod = (UMethod) target.getElement();
            PsiClass psiClass = PsiTypesUtil.getPsiClass(tMethod.getElement().getFunctionalInterfaceType());

            //System.out.println(psiClass.getLanguage().getID());
            PsiClassType.ClassResolveResult generics = PsiUtil.resolveGenericsClassInType(tMethod.getElement().getFunctionalInterfaceType());
