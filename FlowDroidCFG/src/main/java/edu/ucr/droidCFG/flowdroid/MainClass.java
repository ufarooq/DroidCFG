...
public class MainClass {
  ...
  private static void initializeCommandLineOptions() {
    ...
    options.addOption(
        OPTION_PLATFORMS_DIR,
        "platformsdir",
        true,
        "Path to the platforms directory from the Android SDK");
    options.addOption(
        OPTION_SOURCES_SINKS_FILE,
        "sourcessinksfile",
        true,
        "Definition file for sources and sinks");
    options.addOption(
        OPTION_OUTPUT_FILE, "outputfile", true, "Output XML file for the discovered data flows");
    options.addOption(
        OPTION_ADDITIONAL_CLASSPATH,
        "additionalclasspath",
        true,
        "Additional JAR file that shal be put on the classpath");
    options.addOption(
        OPTION_SKIP_APK_FILE,
        "skipapkfile",
        true,
        "APK file to skip when processing a directory of input files");

    options.addOption(OPTION_RETURN_DEPTH, "returndepth", true, "Return results depth");

    options.addOption(OPTION_ACCESS_PATH_SIZE, "accesspathsize", true, "Return results depth");
    // Timeouts
    options.addOption(OPTION_TIMEOUT, "timeout", true, "Timeout for the main data flow analysis");
    options.addOption(
        OPTION_CALLBACK_TIMEOUT,
        "callbacktimeout",
        true,
        "Timeout for the callback collection phase");
    options.addOption(
        OPTION_RESULT_TIMEOUT, "resulttimeout", true, "Timeout for the result collection phase");

    // Optional features
    options.addOption(OPTION_NO_STATIC_FLOWS, "nostatic", false, "Do not track static data flows");
    options.addOption(
        OPTION_NO_CALLBACK_ANALYSIS, "nocallbacks", false, "Do not analyze Android callbacks");
    options.addOption(
        OPTION_NO_EXCEPTIONAL_FLOWS,
        "noexceptions",
        false,
        "Do not track taints across exceptional control flow edges");
    options.addOption(
        OPTION_NO_TYPE_CHECKING,
        "notypechecking",
        false,
        "Disable type checking during taint propagation");
    options.addOption(
        OPTION_REFLECTION, "enablereflection", false, "Enable support for reflective method calls");
    options.addOption(
        OPTION_MISSING_SUMMARIES_FILE,
        "missingsummariesoutputfile",
        true,
        "Outputs a file with information about which summaries are missing");

    // Taint wrapper
    options.addOption(
        OPTION_TAINT_WRAPPER,
        "taintwrapper",
        true,
        "Use the specified taint wrapper algorithm (NONE, EASY, STUBDROID, MULTI)");
    options.addOption(
        OPTION_TAINT_WRAPPER_FILE,
        "taintwrapperfile",
        true,
        "Definition file for the taint wrapper");

    // Individual settings
    options.addOption(OPTION_ACCESS_PATH_LENGTH, "aplength", true, "Maximum access path length");
    options.addOption(
        OPTION_NO_THIS_CHAIN_REDUCTION,
        "nothischainreduction",
        false,
        "Disable reduction of inner class chains");
    options.addOption(
        OPTION_FLOW_INSENSITIVE_ALIASING,
        "aliasflowins",
        false,
        "Use a flow-insensitive alias analysis");
    options.addOption(
        OPTION_COMPUTE_PATHS,
        "paths",
        false,
        "Compute the taint propagation paths and not just source-to-sink connections. This is a shorthand notation for -pr fast.");
    options.addOption(
        OPTION_LOG_SOURCES_SINKS,
        "logsourcesandsinks",
        false,
        "Write the discovered sources and sinks to the log output");
    options.addOption(
        "mt", "maxthreadnum", true, "Limit the maximum number of threads to the given value");
    options.addOption(
  ...
  private void run(String[] args) throws Exception {
    ...
    try {
      ...
      final InfoflowAndroidConfiguration config =
          configFile == null || configFile.isEmpty()
      ...
      if (!targetFile.exists()) {
        System.err.println(
            String.format("Target APK file %s does not exist", targetFile.getCanonicalPath()));
        return;
      }
      ...
      } else apksToAnalyze = Collections.singletonList(targetFile);
      ...
      if (outputFileStr != null && !outputFileStr.isEmpty()) {
        ...
        } else if (apksToAnalyze.size() > 1) outputFile.mkdirs();
      }
      ...
      for (File apkFile : apksToAnalyze) {
        try {
          ...
          if (filesToSkip.contains(apkFile.getName())) {
            logger.info(
                String.format(
                    "Skipping app %s (%d of %d)...",
                    apkFile.getCanonicalPath(), curAppIdx++, apksToAnalyze.size()));
            continue;
          }
          logger.info(
              String.format(
                  "Analyzing app %s (%d of %d)...",
                  apkFile.getCanonicalPath(), curAppIdx++, apksToAnalyze.size()));
          ...
          //        new EasyTaintWrapper(configPath + File.separator +
                      ...
                      new SceneTransformer() {
                        ...
                        protected void internalTransform(String s, Map<String, String> map) {
                          ...
                          BackwardsInterproceduralCFG backwardICFG =
                              new BackwardsInterproceduralCFG(icfg);
                          ...
                          try {
                            manifest =
                                new ProcessManifest(apkFile.toPath().toAbsolutePath().toString());
                          ...
                          for (SootMethod entry : Scene.v().getEntryPoints()) {
                            SootClass dummyClass =
                                entry
                                    .getDeclaringClass(); // Scene.v().getSootClassUnsafe("dummyMainClass");
                            ...
                            for (SootMethod sootMethod : dummyClass.getMethods()) {
                              System.out.println(
                                  "DummyClass Method: " + sootMethod.getSubSignature());
                              if (!sootMethod.getReturnType().equals(VoidType.v())) {
                                SootClass returnClass =
                                    Scene.v().getSootClass(sootMethod.getReturnType().toString());
                          ...
                          while (itCls.hasNext()) {
                            ...
                            if (sootClass.isApplicationClass()
                                && (!sootClass.getPackageName().startsWith("java.")
                                    && !sootClass.getPackageName().startsWith("android."))) {
                              String fileName =
                                  SourceLocator.v()
                                      .getFileNameFor(
                                          sootClass, soot.options.Options.output_format_class);
                          ...
                          Utils.createJarArchive(
                              new File(
                                  outputDirFile.getAbsolutePath() + File.separator + "out.jar"),
                              files);
                          files.parallelStream().forEach(f -> f.deleteOnExit());
                          // String generateJar = "jar cf " + outputDirFile.getAbsolutePath() +
                          // File.separator + "out.jar " + outputDirFile.getAbsolutePath() +
                          // File.separator + "*";
      ...
      System.err.println(
          String.format("The data flow analysis has failed. Error message: %s", e.getMessage()));
      e.printStackTrace();
    }
  }
  ...
  public static FlowDroidJarResult convertToJar(
      String androidPlatforms, File apkFile, String outputDirectory, boolean debug) {
    EntryPointsCreator entryPointCreator = createFlowDroidCFG(androidPlatforms, apkFile);
    JimpleBasedInterproceduralCFG icfg = new JimpleBasedInterproceduralCFG();
    Set<String> entryClasses = new HashSet<>();
    for (SootMethod entry : Scene.v().getEntryPoints()) {
      SootClass dummyClass =
          entry.getDeclaringClass(); // Scene.v().getSootClassUnsafe("dummyMainClass");
      dummyClass.setModifiers(Modifier.PUBLIC);
      ...
      if (!dummyClass.declaresMethodByName("main")) {
        ...
        for (Unit call : calls) {
          Collection<SootMethod> callees = icfg.getCalleesOfCallAt(call);
          for (SootMethod callee : callees) {
            System.out.println("Main Callee: " + callee.getSubSignature());
          }
        }
      }
      if (debug) {
    ...
    while (itCls.hasNext()) {
      ...
      if (sootClass.isApplicationClass()
          && (!sootClass.getPackageName().startsWith("java.")
              && !sootClass.getPackageName().startsWith("android."))) {
        String fileName =
            SourceLocator.v().getFileNameFor(sootClass, soot.options.Options.output_format_class);
        OutputStream streamOut = null;
        try {
          System.out.println(fileName);
          File file = new File(fileName);
    ...
    String jarOutput = executeCommand("jar cvf " + jarPath + " -C " + tmpDir + " .");
    if (debug) System.out.println(jarOutput);
    String outputDir = soot.options.Options.v().output_dir();
    File outputDirFile = new File(outputDir);
    Collection<File> files =
        FileUtils.listFiles(
    ...
    if (!debug) files.parallelStream().forEach(f -> f.deleteOnExit());
  ...
  public static void analyzeAPK(File apkFile, InfoflowAndroidConfiguration config) {
    try {
      ...
      // List<File> classFiles = Arrays.asList(files);
      Utils.createJarArchive(
          new File(outputDirFile.getAbsolutePath() + File.separator + "out.jar"), files);
      files.parallelStream().forEach(f -> f.deleteOnExit());
      // String generateJar = "jar cf " + outputDirFile.getAbsolutePath() + File.separator +
      // "out.jar " + outputDirFile.getAbsolutePath() + File.separator + "*";
      // String generateJarCommandOutput = Utils.executeCommand(generateJar);
      // System.out.println(generateJarCommandOutput);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private static LayoutMatchingMode parseLayoutMatchingMode(String layoutMode) {
    if (layoutMode.equalsIgnoreCase("NONE")) return LayoutMatchingMode.NoMatch;
    else if (layoutMode.equalsIgnoreCase("PWD")) return LayoutMatchingMode.MatchSensitiveOnly;
    else if (layoutMode.equalsIgnoreCase("ALL")) return LayoutMatchingMode.MatchAll;
    else {
  ...
  private static CallbackAnalyzer parseCallbackAnalyzer(String callbackAnalyzer) {
    if (callbackAnalyzer.equalsIgnoreCase("DEFAULT")) return CallbackAnalyzer.Default;
    else if (callbackAnalyzer.equalsIgnoreCase("FAST")) return CallbackAnalyzer.Fast;
    else {
      System.err.println(
          String.format("Invalid callback analysis algorithm: %s", callbackAnalyzer));
      throw new AbortAnalysisException();
    }
  }

  private static DataFlowSolver parseDataFlowSolver(String solver) {
    if (solver.equalsIgnoreCase("CONTEXTFLOWSENSITIVE")) return DataFlowSolver.ContextFlowSensitive;
    else if (solver.equalsIgnoreCase("FLOWINSENSITIVE")) return DataFlowSolver.FlowInsensitive;
    else {
  ...
  private static AliasingAlgorithm parseAliasAlgorithm(String aliasAlgo) {
    if (aliasAlgo.equalsIgnoreCase("NONE")) return AliasingAlgorithm.None;
    else if (aliasAlgo.equalsIgnoreCase("FLOWSENSITIVE")) return AliasingAlgorithm.FlowSensitive;
    else if (aliasAlgo.equalsIgnoreCase("PTSBASED")) return AliasingAlgorithm.PtsBased;
    else if (aliasAlgo.equalsIgnoreCase("LAZY")) return AliasingAlgorithm.Lazy;
    else {
  ...
  private static CodeEliminationMode parseCodeEliminationMode(String eliminationMode) {
    if (eliminationMode.equalsIgnoreCase("NONE")) return CodeEliminationMode.NoCodeElimination;
  ...
  private static CallbackSourceMode parseCallbackSourceMode(String callbackMode) {
    if (callbackMode.equalsIgnoreCase("NONE")) return CallbackSourceMode.NoParametersAsSources;
    else if (callbackMode.equalsIgnoreCase("ALL")) return CallbackSourceMode.AllParametersAsSources;
    else if (callbackMode.equalsIgnoreCase("SOURCELIST")) return CallbackSourceMode.SourceListOnly;
    else {
  ...
  private static PathReconstructionMode parsePathReconstructionMode(String pathReconstructionMode) {
    if (pathReconstructionMode.equalsIgnoreCase("NONE")) return PathReconstructionMode.NoPaths;
    else if (pathReconstructionMode.equalsIgnoreCase("FAST")) return PathReconstructionMode.Fast;
    else if (pathReconstructionMode.equalsIgnoreCase("PRECISE"))
      return PathReconstructionMode.Precise;
    else {
      System.err.println(
          String.format("Invalid path reconstruction mode: %s", pathReconstructionMode));
      throw new AbortAnalysisException();
    }
  }

  private static ImplicitFlowMode parseImplicitFlowMode(String implicitFlowMode) {
    if (implicitFlowMode.equalsIgnoreCase("NONE")) return ImplicitFlowMode.NoImplicitFlows;
    else if (implicitFlowMode.equalsIgnoreCase("ARRAYONLY")) return ImplicitFlowMode.ArrayAccesses;
    else if (implicitFlowMode.equalsIgnoreCase("ALL")) return ImplicitFlowMode.AllImplicitFlows;
    else {
  ...
  private static StaticFieldTrackingMode parseStaticFlowMode(String staticFlowMode) {
    if (staticFlowMode.equalsIgnoreCase("NONE")) return StaticFieldTrackingMode.None;
  ...
  /**
   * Parses the given command-line options and fills the given configuration object accordingly
   *
  ...
  private static void parseCommandLineOptions(
      CommandLine cmd, InfoflowAndroidConfiguration config, Set<String> filesToSkip) {
    // Files
    {
...

    {
      ...
      if (depth != null) Configuration.v().setReturnDepth(depth);
    }

    {
      Integer size = getIntOption(cmd, OPTION_ACCESS_PATH_SIZE);
      if (size != null) Configuration.v().setAccessPathSize(size);
    }
    // Timeouts
    {
      Integer timeout = getIntOption(cmd, OPTION_TIMEOUT);
      if (timeout != null) config.setDataFlowTimeout(timeout);
    }
    {
      Integer timeout = getIntOption(cmd, OPTION_CALLBACK_TIMEOUT);
      if (timeout != null) config.getCallbackConfig().setCallbackAnalysisTimeout(timeout);
    }
    {
      Integer timeout = getIntOption(cmd, OPTION_RESULT_TIMEOUT);
      if (timeout != null) config.getPathConfiguration().setPathReconstructionTimeout(timeout);
    }
    ...
    if (cmd.hasOption(OPTION_NO_EXCEPTIONAL_FLOWS)) config.setEnableExceptionTracking(false);
    if (cmd.hasOption(OPTION_NO_TYPE_CHECKING)) config.setEnableTypeChecking(false);
    if (cmd.hasOption(OPTION_REFLECTION)) config.setEnableReflection(true);
    // Individual settings
    {
      Integer aplength = getIntOption(cmd, OPTION_ACCESS_PATH_LENGTH);
      if (aplength != null) config.getAccessPathConfiguration().setAccessPathLength(aplength);
    }
    if (cmd.hasOption(OPTION_NO_THIS_CHAIN_REDUCTION))
      config.getAccessPathConfiguration().setUseThisChainReduction(false);
    if (cmd.hasOption(OPTION_FLOW_INSENSITIVE_ALIASING)) config.setFlowSensitiveAliasing(false);
    if (cmd.hasOption(OPTION_COMPUTE_PATHS))
      config.getPathConfiguration().setPathReconstructionMode(PathReconstructionMode.Fast);
    if (cmd.hasOption(OPTION_ONE_SOURCE)) config.setOneSourceAtATime(true);
    if (cmd.hasOption(OPTION_ONE_COMPONENT)) config.setOneComponentAtATime(true);
    if (cmd.hasOption(OPTION_SEQUENTIAL_PATHS))
      config.getPathConfiguration().setSequentialPathProcessing(true);
    if (cmd.hasOption(OPTION_LOG_SOURCES_SINKS)) config.setLogSourcesAndSinks(true);
    if (cmd.hasOption(OPTION_MERGE_DEX_FILES)) config.setMergeDexFiles(true);
    ...
    }
    {
      ...
      if (maxDepth != null) config.getCallbackConfig().setMaxAnalysisCallbackDepth(maxDepth);
    }

    // Inter-component communication
    if (cmd.hasOption(OPTION_ICC_NO_PURIFY)) config.getIccConfig().setIccResultsPurify(false);
    {
      String iccModel = cmd.getOptionValue(OPTION_ICC_MODEL);
      if (iccModel != null && !iccModel.isEmpty()) config.getIccConfig().setIccModel(iccModel);
    }
    ...
    }
    {
      ...
      if (pathAlgo != null && !pathAlgo.isEmpty())
        config
            ...
            .setPathBuildingAlgorithm(parsePathReconstructionAlgo(pathAlgo));
    }
    {
    ...
    }
    {
      ...
      if (pathMode != null && !pathMode.isEmpty())
        config
            .getPathConfiguration()
            .setPathReconstructionMode(parsePathReconstructionMode(pathMode));
    }
    {
...

    {
      ...
      if (toSkip != null && toSkip.length > 0) {
        for (String skipAPK : toSkip) filesToSkip.add(skipAPK);
      }
    }
   ...
   *
   * @param configFile The configuration file from which to load the data flow configuration
